...
export async function GET(
  ...
  try {
    const { id } = params // Moved id declaration to the top
    console.log("API received student ID:", id) // Keep console.log
    ...
    if (session.user.role === "STUDENT" && session.user.nis !== id) {
      return NextResponse.json({ success: false, message: "Unauthorized: Students can only view their own data." }, { status: 403 })
    }
    ...
    console.error("[API/STUDENT/ID] Error fetching student details:", error)
