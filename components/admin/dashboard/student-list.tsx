...
import { useState, useMemo, useCallback } from 'react' // Removed useEffect as it's handled by parent
...
export const StudentList = React.memo(function StudentList({
  ...
  const handleJurusanFilterChange = useCallback((jurusan: string) => {
    setFilterJurusan(jurusan);
  }, [setFilterJurusan]);

  const handleAngkatanFilterChange = useCallback((angkatan: string) => {
